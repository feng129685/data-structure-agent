...
<template>
  ...
  >
    ...
    <form class="admin-command-row admin-toolbar" @submit.prevent="applyFilters">
      <div class="admin-command-row__label"><strong>筛选</strong></div>
    ...
    <section v-else class="admin-data-surface admin-table-wrap admin-motion-enter" aria-label="后台任务">
      <header class="admin-data-surface__head"><h2>任务列表</h2><span class="admin-data-surface__hint">requestId 全部来自服务端</span></header>
      <table class="admin-table admin-data-table admin-task-table">
        ...
        <tbody>
          <tr v-for="task in items" :key="task.id" class="admin-data-row">
    ...
    <aside v-if="selected" class="admin-inspector admin-detail admin-motion-enter" aria-label="任务详情">
      <div class="admin-detail__header admin-inspector__header"><div><p class="admin-kicker">任务详情</p><h2>任务 #{{ selected.id }}</h2><p class="admin-code">{{ selected.taskType }}</p></div><button class="button button--small" type="button" @click="closeDetail">关闭</button></div>
