...
<template>
  ...
  >
    ...
    <section v-else class="admin-data-surface admin-table-wrap admin-motion-enter" aria-label="审核队列">
      <header class="admin-data-surface__head"><div><p class="admin-kicker">审核清单</p><h2>待核验项目</h2></div><span class="admin-data-surface__hint">来源链完整性由服务端最终校验</span></header>
      <table class="admin-table admin-data-table">
        <thead><tr><th>项目</th><th>状态</th><th>来源链</th><th>更新时间</th><th>操作</th></tr></thead>
        <tbody>
          <tr v-for="item in items" :key="`${item.type}-${item.id}`" class="admin-data-row">
    ...
    <aside v-if="selected" class="admin-inspector admin-detail admin-motion-enter">
      <div class="admin-detail__header admin-inspector__header"><div><p class="admin-kicker">项目详情</p><h2>{{ selected.item.title }}</h2><p class="admin-code">{{ selected.item.type }} · {{ selected.item.id }}</p></div><button class="button button--small" type="button" @click="closeDetail">关闭</button></div>
